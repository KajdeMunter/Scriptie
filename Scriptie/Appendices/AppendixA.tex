...
\section{Grafana implementatie} \label{GrafanaImplementatie}
docker/docker-compose.proxy.prod.yml
\begin{minted}[linenos=true, bgcolor=codebg, breaklines]{yaml}
  grafana:
    image: "grafana/grafana:6.4.1"
...
- import_tasks: "publish.yml"
- import_tasks: "evaluate_policies.yml"
- import_tasks: "publish.yml"
\end{minted}
...
secure-daemon/tasks/evaluate\_policies.yml
\begin{minted}[linenos=true, bgcolor=codebg, breaklines]{yaml}
- name: Ensure policy directory
  file:
...
- name: Ensure policies
  template:
    src: etc/docker/policies/authz.rego.j2
...
- name: Ensure .docker directories
  ...
  loop:
    - "/home/{{ default_user }}"
    - "/home/{{ default_users[1].name }}"
    - "/root"

- name: Ensure docker http headers
  template:
    src: config.json.j2
  ...
  loop:
    - "{{ default_user }}"
    - "{{ default_users[1].name }}"

- name: Ensure root docker http headers
  template:
    src: config.json.j2
...
- name: Install OPA plugin
  command: 'docker plugin install --grant-all-permissions openpolicyagent/opa-docker-authz-v2:0.4 opa-args="-policy-file /opa/policies/authz.rego"'
\end{minted}
    ...
    - step:
        ...
        script:
          ...
          - 'ansible-playbook
            ...
            -e nginx_tag=website-latest
            -e traefik_tag=website-latest
            -e clone_dir=$BITBUCKET_CLONE_DIR
            -i ansible/inventories/local.yml
            --vault-password-file=../.vault-password
            -e add_latest_tag=false
            --tags=build_images ansible/site.yml'
