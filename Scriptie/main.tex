...
\usepackage[backend=biber,style=ieee,dashed=false]{biblatex} % Use bibtext and IEEE reference style
...
\thesistitle{Verbetering van schaal- en onderhoudbaarheid in de infrastructuur van Developers.nl} % Your thesis title, this is used in the title and abstract, print it elsewhere with \ttitle
...
\version{v0.6} % Version number, used in omslag and titelblad, print it with \versionnr
...
\hoofdvraag{Op welke wijze kan Developers.nl de architectuur van haar websites beter schaal- en onderhoudbaar maken?}
\deelvraagverwachtingen{Wat zijn de wensen en eisen van Developers.nl met betrekking tot de schaal- en onderhoudbaarheid van haar huidige websites?}
\deelvraagtechnieken{Welke standaarden en best-practices voor het waarborgen van schaal- en onderhoudbaarheid zijn relevant voor de eisen van Developers.nl?}
\deelvraaghuidig{Hoe onderhoud- en schaalbaar zijn de huidige websites van Developers.nl met betrekking tot de relevante kwaliteitsstandaarden?}
\deelvraagverbetering{Wat voor verbeteringen ten aanzien van schaal- en onderhoudbaarheid kunnen worden toegepast op de huidige websites van Developers.nl?}
\deelvraagimplementatie{Hoe kunnen de gekozen verbeteringen ten aanzien van schaal- en onderhoudbaarheid geïmplementeerd worden?}
...
Feature environments zijn omgevingen die individueel gedeployed kunnen worden voor elke individuele nieuwe feature die wordt toegevoegd. Dit is gerealiseerd door middel van een reverse proxy, Traefik.

Er is onderzoek gedaan naar relevante kwaliteitsstandaarden om schaal- en onderhoudbaarheid te waarborgen. Met de gevonden standaarden zijn een aantal technieken geïmplementeerd. Codecov is geïmplementeerd om code-coverage te waarborgen, Docker images worden periodiek opgeruimd om ruimte op de server vrij te houden, Grafana is geïmplementeerd om monitoring data te visualiseren, en Open Policy Agent is gebruikt om beveiliging en kwaliteit van nieuwe toevoegingen te waarborgen. 
...
Feature environments are environments that can be deployed individually for each individual new feature that is added. This has been achieved through a reverse proxy, Traefik.

Research has been conducted to find relevant quality standards in order to guarantee scalability and maintainability. A number of techniques have been implemented with the found standards. Codecov has been implemented to ensure code coverage, Docker images are periodically cleaned up to keep space on the server, Grafana has been implemented to visualize monitoring data, and Open Policy Agent has been used to enforce the security and quality of new additions.

Furthermore, a recommendation was made on the use of cloud service providers, serverless computing, container orchestration, policies, and the development of a generic infrastructure.
\end{abstract}
...
\begin{begrippen}{p{3cm}|p{10cm}} % Include a list of begrippen (a table of two columns)
...
\include{Chapters/TheoretischKader}
\include{Chapters/Verwachtingen}
\include{Chapters/Technieken}
\include{Chapters/Huidigesituatie}
\include{Chapters/Verbeteringen} 
\include{Chapters/Implementatie} 
\include{Chapters/Requirements}
\include{Chapters/Aanbevelingen}
\include{Chapters/Conclusie}
\include{Chapters/Reflectie}
