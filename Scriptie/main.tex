...
\version{v1.0} % Version number, used in omslag and titelblad, print it with \versionnr
...
Feature-environments are environments that can be deployed individually for each new feature that is added. This has been achieved through a reverse proxy, Traefik.

Research has been conducted for relevant quality standards to guarantee scalability and maintainability. A number of techniques have been implemented using the found standards. Codecov to ensure code coverage, Grafana to visualize monitoring data, and Open Policy Agent to ensure security and quality of new additions. In addition, Docker images are periodically cleaned up to keep space on the server free.
